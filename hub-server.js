...
// ── Proxy targets (internal Docker IPs) ──────────────────────
const PROXY_TARGETS = {
  arden:   { host: '10.0.1.9',  port: 3003 },
  ops:     { host: '10.0.1.6',  port: 3001 },
...
const server = http.createServer(async (req, res) => {
  ...
  // Proxy /api/* and other dashboard calls using session cookie
  ...
  if (req.method === 'POST' && req.url === '/auth/login') {
    try {
      ...
      // Set cookie so API calls know which dashboard to proxy to
